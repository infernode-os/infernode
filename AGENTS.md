...
| Wi-Fi: `crypt off` clears the firmware's WPA state (open networks join, #638); the supplicant re-issues the join while unassociated (re-join after an AP outage) | `tests/acceptance/wifi.py` with `wifi-ap.sh` on the tester -- the only test there is, since the radio is hardware | any change to `os/bcm/ether4330.c` or `appl/cmd/ip/wpa.b` |
