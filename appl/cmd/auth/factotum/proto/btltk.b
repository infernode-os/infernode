...
#
#		key proto=btltk addr=c8:f3:e8:06:55:8b type=1 ediv=4660 rand=0102030405060708 irk=... !ltk=0123...cdef
#
#	type is the peer's address type (0 public, 1 random); irk, when
#	the peer gave one, is what resolves the private addresses it
#	advertises with once bonded, and is not secret in the sense the
#	LTK is -- it identifies, it does not encrypt.  As with
...
interaction(attrs: list of ref Attr, io: ref Authio->IO): string
{
	...
	irk := authio->lookattrval(key.attrs, "irk");
	if(irk == nil)
		irk = "-";
	# what bt9p reads back: the key, the EDIV, the Rand, the IRK or -
	a := array of byte (k + " " + ediv + " " + rnd + " " + irk);
	io.write(a, len a);
	return nil;
}
