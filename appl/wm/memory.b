...
{
	# Run in-proc rather than spawning: the presentation launcher already
	# runs each app in its own process and tracks it by the init proc, so a
	# spawn that lets init return immediately tore the app's slot down before
	# its window ever joined the zone wmsrv (no window appeared).
	realinit(ctxt);
}
