...
init(ctxt: ref Draw->Context, argv: list of string)
{
	...
	# Bound the board by the available display, but only trust the display
	# image once it is genuinely laid out.  When tetris runs embedded in the
	# lucifer presentation zone, ctxt.display.image is a tiny placeholder
	# (~12x8) before the real window attaches; using it would collapse the
	# board to ~1px (dx <= 0).  Require room for a full block-sized board
	# and otherwise keep the generous default so the board caps at blocksize.
	...
	if (ctxt.display.image != nil) {
		...
		my := img.r.dy() - wsz.y;
		mx := img.r.dx();
		if (mx >= 2 * BOARDWIDTH * blocksize && my >= BOARDHEIGHT * blocksize) {
			maxsize.x = mx;
			maxsize.y = my;
		}
	}
	...
	# Keep the toplevel at its window size instead of shrink-wrapping to the
	# board.  Embedded in the lucifer presentation zone the window is large
	# (e.g. 677x844); with geometry propagation on, "." collapses to the
	# board's natural size and Tk draws it at a stale origin, so the board
	# lands off-window.  Pinning the size (as the classic score screen already
	# did) lets us anchor the board top-centre in the window below.
