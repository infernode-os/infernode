...
|----------------------|-----------|----------|--------|
| Hardware authenticator, secret non-exportable | YubiKey FIDO2; `hmac-secret` output `R` derived **on-key**, never leaves it | `doc/second-factor-auth.md` §1, §6; `doc/yubikey-2fa-operations.md` §9 | ✅ |
...
| Phishing / verifier-impersonation resistance | Challenge-response bound to credential; not a typed/replayable shared secret | `doc/second-factor-auth.md` §4–§6 | ✅ |
| User verification (UV) | FIDO2 **PIN** verified on-key; UV output cryptographically distinct from touch-only | `doc/yubikey-2fa-operations.md` §intro, §9, §11 (`t2uv`) | ✅ hardware-verified |
| Credential bound, deterministic per (cred, salt) | `hmac-secret` assertion mixed into the secstore file-key | `doc/second-factor-auth.md` §6 | ✅ |
...
(`doc/yubikey-2fa-operations.md` §11). This is the same evidence that substantiates the
...
challenge-response** instead (`doc/second-factor-auth.md` §2 "Non-goals"). The FIDO2/CTAP2
...
- `doc/second-factor-auth.md`, `doc/yubikey-2fa-operations.md`.
