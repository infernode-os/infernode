...
### win_flex_bison (required)

The Limbo compiler's parser is generated by bison from `limbo/limbo.y`. The Windows builds ship a port called **win_flex_bison** (provides `win_bison.exe`).
...
**Important:** `winget` and `choco` do not refresh PATH in the shell that ran the install. After installing, **open a new x64 Native Tools Command Prompt for VS 2022** before invoking the build, or run `build-windows-amd64.ps1` from a fresh shell. The build script auto-detects WinGet shim (`%LOCALAPPDATA%\Microsoft\WinGet\Links`), choco bin, and scoop shims, but it can only find them if PATH is fresh or the shim directories themselves are intact.

### POSIX host shell (recommended)

The Inferno runtime relies on a host POSIX `sh.exe` for a small number of integrations (process env passthrough, host-side helpers). On Windows the simplest options are:

- **Git for Windows** — `winget install Git.Git` (ships `sh.exe` in `C:\Program Files\Git\bin`)
- **MSYS2** — `winget install MSYS2.MSYS2`
- **Cygwin** — https://cygwin.com/install.html

After installing, ensure the shell's `bin/` directory is on PATH. Without it, you'll see warnings like `os: cannot exec ...` during boot. `setup-windows.ps1` checks for `sh.exe` and surfaces a hint if it's missing.
