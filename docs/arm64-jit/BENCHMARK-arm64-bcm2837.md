...
- **OS:** InferNode native kernel (`os/bcm2837`), master fe02e8bf9 + PR #707 (the in-module return fix for #689), 1,806,141 bytes; the Dis VM runs in the kernel, no host OS
- **Date:** 2026-09-24 (JIT runs; the interpreter runs are from 2026-09-23 on the previous kernel, which #707 does not change when the JIT is off)
...
|-----|-------------|----------|---------|
| 1 | 273,572 | 37,820 | 7.23x |
| 2 | — | 37,816 | — |
| 3 | — | 37,820 | — |
| **Best** | **273,572** | **37,816** | **7.23x** |
...
| **Memory Access** | **1,357** | **186** | **7.30x** |
| 3a. Sequential read | 241 | 34 | 7.09x |
| 3b. Sequential write | 253 | 33 | 7.67x |
| 3c. Stride access | 447 | 63 | 7.10x |
| 3d. Small array hot | 416 | 56 | 7.43x |
| **Function Calls** | **5,287** | **1,006** | **5.26x** |
| 4a. Simple call | 232 | 25 | 9.28x |
| 4b. Recursive fib | 4,469 | 878 | 5.09x |
| 4c. Mutual recursion | 273 | 48 | 5.69x |
| 4d. Deep call chain | 313 | 55 | 5.69x |
...
| **Byte Ops** | **1,108** | **116** | **9.55x** |
| 6a. Byte arithmetic | 379 | 34 | 11.15x |
| 6b. Byte array | 729 | 82 | 8.89x |
| **List Ops** | **340** | **99** | **3.43x** |
| 7a. List build | 82 | 53 | 1.55x |
| 7b. List traverse | 258 | 46 | 5.61x |
...
**Emu** is the hosted InferNode emulator (`emu/Linux/o.emu`, a headless Linux arm64 build of master as of 2026-09-23, without #707: its function-call rows are therefore the pre-fix JIT, and the bare ÷ emu figures on those rows measure the fix, not the environment) running under Raspberry Pi OS Lite (Trixie, 2026-09-15, 64-bit, kernel 6.18) on the same board from a second SD card, the same afternoon, its cpufreq governor at 1400 MHz throughout (sampled). Same two suites, same protocol: best of 3 JIT, one interpreter run, ms. "Bare" is the native kernel: no host OS, the Dis VM in the kernel.
...
| Loop with Array Access | 241,981 | 33,564 | 7.2x | 192,467 | 29,043 | 6.6x | 1.16 |
| Function Calls | 288 | 31 | 9.3x | 240 | 27 | 8.9x | 1.15 |
| Fibonacci (recursive) | 8,950 | 1,742 | 5.1x | 12,021 | 6,056 | 2.0x | 0.29 |
| Sieve of Eratosthenes | 967 | 134 | 7.2x | 797 | 124 | 6.4x | 1.08 |
| Nested Loops | 14,580 | 1,517 | 9.6x | 11,953 | 1,312 | 9.1x | 1.16 |
| **v1 total** | **273,572** | **37,816** | **7.2x** | **222,879** | **37,283** | **6.0x** | **1.01** |
| v2 Integer ALU | 1,765 | 208 | 8.5x | 1,543 | 181 | 8.5x | 1.15 |
| v2 Branch & Control | 902 | 99 | 9.1x | 717 | 87 | 8.2x | 1.14 |
| v2 Memory Access | 1,357 | 186 | 7.3x | 1,167 | 161 | 7.2x | 1.16 |
| v2 Function Calls | 5,287 | 1,006 | 5.3x | 6,926 | 9,503 | 0.7x | 0.11 |
| v2 Big (64-bit) | 1,563 | 154 | 10.1x | 1,347 | 132 | 10.2x | 1.17 |
| v2 Byte Ops | 1,108 | 116 | 9.6x | 899 | 100 | 9.0x | 1.16 |
| v2 List Ops | 340 | 99 | 3.4x | 333 | 102 | 3.3x | 0.97 |
| v2 Mixed Workloads | 4,802 | 672 | 7.1x | 4,039 | 582 | 6.9x | 1.15 |
| v2 Type Conversions | 622 | 59 | 10.5x | 498 | 50 | 10.0x | 1.18 |
| **v2 total** | **17,752** | **2,607** | **6.8x** | **17,475** | **10,929** | **1.6x** | **0.24** |
...
**Recursion (#689, fixed by #707).** Until 2026-09-24 every return within a module -- every return of a recursive function -- left compiled code for a C call to `OP(ret)` and a dispatch through `R.PC`, so recursion under the JIT ran no faster than the interpreter (recursive fib 5,162 ms against the interpreter's 4,469). `macret` now calls destroy and branches to the frame's `lr` inline for that case, as amd64's always has. On this board: recursive fib 5,162 → 878 ms, mutual recursion 238 → 48, deep call chain 264 → 55, the v2 total 7,354 → 2,607 ms; every other item unchanged to the millisecond. The same fix on the hosted Linux/arm64 emulator on a Jetson AGX Orin, same day, before and after: v2 1,970 → 897 ms, recursive fib 1,611 → 620 (its interpreter: 988), so the hosted numbers elsewhere in `docs/BENCHMARKS.md` that predate #707 understate the JIT on every call-heavy item.
...
Raw output for every run named here -- three JIT and one interpreter run of each suite, bare and emu -- is kept with the bench evidence (`infernode-bench/benchmarks/2026-09-23-*.txt` and `2026-09-24-triage-689-1400MHz-jit-x3.txt` on the bench store; the Jetson before/after pair is `2026-09-24-jetson-hosted-{before,after}-707.txt`). The kernel is the one whose 48-hour soak began the same evening, after the batteries and a ten-minute storm passed on it.
