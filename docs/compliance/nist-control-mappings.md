...
pass in [`../../doc/compliance/SP800-53-171-mapping.md`](../../doc/compliance/SP800-53-171-mapping.md)
...
|---------|--------------------------|-----------------------------|--------------------|---------------|------------------|-------|
| **AC-3** / 3.1.1–3.1.2 | Enforce approved authorizations for access to resources | Per-process namespace **is** the capability set; a name not bound cannot be expressed. Single mediation path (`namec`). | By construction | `emu/port/chan.c`; `../../doc/compliance/SP800-207-zero-trust.md` | FDP_ACC.2, FDP_ACF.1 | Enforcement is structural, not a checkpoint |
| **AC-4** / 3.1.3 | Control information flow between domains | Namespace boundaries mediate flow; cross-domain *guarded* transfer not yet built | Partial | `emu/port/pgrp.c`; `../../doc/compliance/SP800-53-controls.md` (AC-4) | FDP_IFC.1, FDP_IFF.1 | CDS guard is roadmap (EPIC 5) — **non-compliance gap (F-7)** |
| **AC-5** / 3.1.4 | Separation of duties | Distinct capability sets + distinct factotum identities per subject | Partial | `../../doc/compliance/SP800-53-controls.md` (AC-5) | FMT_SMR.1 | Enforcement partly operator process |
...
| **AC-6(9)** / 3.1.7 | Audit use of privileged functions | Audit log records privileged credential ops (factotum keyadd/keydel, 2fa enroll/disable) | Configurable | `../../doc/compliance/SP800-92-audit-log.md` | FAU_GEN.1 | Broader privileged-op coverage pending — **gap (F-4)** |
...
| Control | Requirement (paraphrase) | How InferNode satisfies it | Satisfaction basis | Evidence path | CC SFR crosswalk | Notes |
|---------|--------------------------|-----------------------------|--------------------|---------------|------------------|-------|
| **AU-2** / 3.3.1 | Log defined events | `auditfs` service + auth/identity/credential emitters | Configurable | `appl/cmd/auditfs.b`; `../../doc/compliance/SP800-92-audit-log.md` §2 | FAU_GEN.1 | Must be bound into namespace to record |
...
| **AU-9(3)** | Cryptographic protection of audit info | Hash chain + signed checkpoints | By construction | `../../doc/compliance/SP800-92-audit-log.md` | FAU_STG.2 | |
...
| Control | Requirement (paraphrase) | How InferNode satisfies it | Satisfaction basis | Evidence path | CC SFR crosswalk | Notes |
|---------|--------------------------|-----------------------------|--------------------|---------------|------------------|-------|
| **IA-2** / 3.5.1 | Uniquely identify & authenticate users | Factotum + secstore login; native STS mutual auth | Configurable | `module/factotum.m`; `../../doc/compliance/SP800-63B-AAL3.md` | FIA_UID.2, FIA_UAU.2 | |
| **IA-2(1)** / 3.5.3 | MFA to privileged accounts | FIDO2 hardware key + user verification (PIN) gating secstore unlock — AAL3 | Configurable | `../../doc/compliance/SP800-63B-AAL3.md`; `../../doc/compliance/FIDO2-CTAP2.md` | FIA_UAU.5, FTP_TRP.1 | AAL3 verifier shipped |
...
| **IA-5** / 3.5.2 | Authenticator management | DK-wrapped secstore slots; recovery slot; never-brick enroll | Partial | `../../doc/compliance/SP800-63B-AAL3.md` §4 | FIA_AFL.1 | Dual-key-by-default / DK save-back open (EPIC 1) |
| **IA-5(2)** | PKI-based auth incl. revocation | X.509 path validation + CRL; PQ-capable certs | Configurable | `appl/lib/crypt/x509.b`; `../../doc/compliance/X509-mTLS.md` | FIA_UAU.1(PKI) | |
...
| **IA-3 / lockout** | Bound failed-attempt exposure | secstore factor gating | Needs confirmation | `../../doc/compliance/SP800-63B-AAL3.md` §4 | FIA_AFL.1 | Lockout thresholds **needs confirmation** — **gap (F-6)** |

## 4. System & Communications Protection (SC) — SP 800-53 / 800-171 §3.13

| Control | Requirement (paraphrase) | How InferNode satisfies it | Satisfaction basis | Evidence path | CC SFR crosswalk | Notes |
|---------|--------------------------|-----------------------------|--------------------|---------------|------------------|-------|
| **SC-7** / 3.13.1 | Boundary protection | Namespace boundaries + 9P as the one mediated interface; `NODEVS` gate | By construction | `emu/port/chan.c`; `../../doc/compliance/SP800-53-controls.md` (SC-7) | FDP_ACF.1, FTP_ITC.1 | |
| **SC-8** / 3.13.8 | Transmission confidentiality & integrity | TLS 1.2/1.3 (AES-256-GCM); native STS line encryption | Configurable | `appl/lib/crypt/tls.b`; `../../doc/compliance/X509-mTLS.md` | FTP_ITC.1 | Client-cert mTLS open (INFR-344) |
...
| **SC-12** / 3.13.10 | Key establishment & management | Hybrid X25519+ML-KEM (TLS), DH+ML-KEM (native); SecP384r1MLKEM1024 under CNSA | Configurable | `appl/lib/crypt/tls.b` (`0x11ED`); `../../doc/compliance/CNSA-2.0.md` | FCS_CKM.1/.2 | Default classical; CNSA mode off by default |
| **SC-13** / 3.13.11 | FIPS-validated cryptography | Approved algorithms present (AES-256, SHA-384/512, FIPS 203/204/205) — **module not FIPS-validated** | Partial | `../../doc/compliance/CNSA-2.0.md`; `../../doc/compliance/FIPS-140-3-readiness.md` | FCS_COP.1(*) | **gap (F-2)**: algorithms ✅ / validated module ☐ |
...
| **SC-28** / 3.13.16 | Protection of information at rest | AES-256-GCM secstore vault, DK-wrapped, factor-gated | Configurable | `../../doc/compliance/SP800-63B-AAL3.md` §1 | FDP_RIP.1 | |
...
| **CM-6** / 3.4.2 | Configuration settings | CNSA mode (`/env/cnsamode`), `NODEVS`, export scope are explicit settings | Configurable | `appl/lib/crypt/tls.b` (`cnsamode`); `../../doc/compliance/CNSA-2.0.md` | FMT_SMF.1 | Secure settings are opt-in — integrator must set |
...
[`../../doc/compliance/SP800-53-171-mapping.md`](../../doc/compliance/SP800-53-171-mapping.md) §3.
...
| **F-2** | Cryptographic module not FIPS 140-2/140-3 CMVP validated | SC-13, IA-7 | Validation | `../../doc/compliance/FIPS-140-3-readiness.md` (readiness only) |
...
- Sibling artifacts: [`../../doc/compliance/SP800-53-171-mapping.md`](../../doc/compliance/SP800-53-171-mapping.md),
  [`../../doc/compliance/SP800-53-controls.md`](../../doc/compliance/SP800-53-controls.md),
  ...
  [`../../doc/compliance/FIPS-140-3-readiness.md`](../../doc/compliance/FIPS-140-3-readiness.md).
