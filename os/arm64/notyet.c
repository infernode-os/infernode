...
/*
 * The device table was here. It names the devices a kernel includes,
 * and which those are is the board's to say -- GPIO, the touch panel,
 * the SD card and the audio jack are a Raspberry Pi's, not AArch64's
 * -- so each board has its own: os/<board>/devtab.c.
 */
 ...
 * math. This is that list, written out, for the ones this kernel
 * builds.
 *
 ...
 *
 * drawmodinit() registers $Draw, the image and screen module. It is the
 * Limbo half of the draw device: devdraw serves the protocol, $Draw is
 * what a program calls to speak it. Registering the module is separate
