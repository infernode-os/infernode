/*
 * Register layouts of the Broadcom/VideoCore peripheral blocks the
 * Raspberry Pi SoCs share: mailbox, the two UARTs, GPIO, the system and
 * ARM timers, power management, both SD controllers.
 *
 * Not included directly. A board's io.h defines PHYSIO -- where the
 * peripheral window is on that SoC -- and the interrupt numbers (IRQusb
 * and friends), which depend on the SoC's interrupt controller, and
 * then includes this. The blocks are the same silicon from the BCM2835
 * to the BCM2711; what moves is the window and how interrupts arrive.
 *
