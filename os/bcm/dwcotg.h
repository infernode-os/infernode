/*
 * Synopsys DesignWare DWC OTG register definitions.
 *
 * IMPORTED VERBATIM from Plan 9 4th edition, sys/src/9/bcm/dwcotg.h, via the
...
/*
 * volatile, and it matters.
 *
 * These are Device-nGnRnE registers: unaligned access faults there
 * regardless of SCTLR_EL1.A, and at -O2 clang merges two adjacent
 * 32-bit stores into a single 64-bit one. Two neighbouring registers
 * written in sequence then land on a 4-but-not-8-aligned address and
 * take a data abort -- observed as a fault writing 0x3f980024.
 *
 * Upstream needs no such marking because the Plan 9 compiler performs
 * no such merge. This is a property of the toolchain, not of the
 * hardware, which is why the rest of the file stays verbatim.
 */
