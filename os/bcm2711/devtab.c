 ...
 *
 * The Raspberry Pi 3B+'s, exactly, today -- every device on that list
 * is a block this SoC also has, driven by the same file in ../bcm. It is
 * a file of this board's and not a shared one because it is where the
 * two will part: a gigabit Ethernet MAC and a PCIe host with a USB 3
 * controller behind it are this board's and have no driver yet.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
Dev*	devtab[] =
{
	...
	&audiodevtab,		/* 'A' -- #A, /dev/audio: the jack, PWM through DMA (os/bcm/audiopwm.c) */
	nil,
};
