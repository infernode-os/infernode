 ...
 * file, which lists the devices this kernel includes. No devices are
 * imported yet, so the table is empty -- but it must exist and must be
 * nil-terminated, because chan.c walks it to resolve a device letter.
 *
 * When devroot.c and devcons.c land this is replaced by the generated
 * $CONF.c, which also emits modinit(). It is here rather than in a
 * "conf.c" precisely so it is obvious it is a placeholder.
 */
Dev*	devtab[] =
{
	nil,
