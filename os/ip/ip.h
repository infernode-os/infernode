/*
 * os/ip data structures.
 *
 * IMPORTED from upstream Inferno (inferno-os/inferno-os), os/ip/ip.h.
...
{
	/*
	 * u32int, not ulong. An IPv4 address IS 32 bits, and saying so is
	 * what keeps the arithmetic honest: with a 64-bit ulong, the
	 * end-address computation "ea = sa | ~m" sets every bit above 32,
	 * and lookup's "addr <= endaddress" then matches addresses the
	 * route does not own. See tests/host/iproute_test.sh.
	 */
	u32int	address;
	u32int	endaddress;
...
{
	/*
	 * u32int, so that IPllen words are exactly IPaddrlen bytes again.
	 * os/ip relies on that identity -- v6addroute builds an address in
	 * one of these arrays and copies it out with a plain
	 * memmove(..., IPaddrlen). With 64-bit words the array is 32 bytes,
	 * the copy takes 16, and every IPv6 route silently loses half its
	 * address.
	 */
	u32int	address[IPllen];
	u32int	endaddress[IPllen];
	uchar	gate[IPaddrlen];
};

/*
 * The identity the code above depends on, checked rather than assumed.
 */
typedef char ipllenmatchesaddrlen[
	(IPllen * sizeof(u32int)) == IPaddrlen ? 1 : -1];
