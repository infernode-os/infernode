/*
 * IMPORTED from upstream Inferno (inferno-os/inferno-os), os/ip/tcp.c.
 ...
 *
 * Being ported to 64-bit. tcp.c is the second file in os/ip with a
 * silent-wrong-answer failure mode, and unlike iproute.c its bug is one
 * of SURVIVAL BY ACCIDENT: the sequence comparisons work under LP64
 * only because an (int) cast truncates 64-bit arithmetic back to 32.
 * That passes any smoke test and stalls under real traffic, which is
 * the worst way for it to surface. tests/host/tcpseq_test.sh exists
...
struct Tcpctl
{
	...
	struct {
		u32int	una;		/* Unacked data pointer */
		u32int	nxt;		/* Next sequence expected */
		u32int	ptr;		/* Data pointer */
		u32int	wnd;		/* Tcp send window */
		u32int	urg;		/* Urgent data pointer */
		u32int	wl2;
		int	scale;		/* how much to right shift window in xmitted packets */
		/* to implement tahoe and reno TCP */
		u32int	dupacks;	/* number of duplicate acks rcvd */
		int	recovery;	/* loss recovery flag */
		u32int	rxt;		/* right window marker for recovery */
	} snd;
	struct {
		u32int	nxt;		/* Receive pointer to next uchar slot */
		u32int	wnd;		/* Receive window incoming */
		u32int	urg;		/* Urgent pointer */
		int	blocked;
		int	una;		/* unacked data segs */
		int	scale;		/* how much to left shift window in rcved packets */
	} rcv;
	u32int	iss;			/* Initial sequence number */
	int	sawwsopt;		/* true if we saw a wsopt on the incoming SYN */
	u32int	cwind;			/* Congestion window */
	...
	u32int	rttseq;			/* Round trip sequence */
...
struct Limbo
{
	...
	ushort	lport;
	ushort	rport;
	u32int	irs;		/* initial received sequence */
	u32int	iss;		/* initial sent sequence */
...
int
seq_within(u32int x, u32int low, u32int high)
{
	if(low <= high){
		if(low <= x && x <= high)
			return 1;
...
int
seq_lt(u32int x, u32int y)
{
	return (int)(x-y) < 0;
}

int
seq_le(u32int x, u32int y)
{
	return (int)(x-y) <= 0;
}

int
seq_gt(u32int x, u32int y)
{
	return (int)(x-y) > 0;
}

int
seq_ge(u32int x, u32int y)
{
	return (int)(x-y) >= 0;
}
...
reset:
	...
	if(tcb->state != Syn_received && (seg.flags & RST) == 0){
		...
		&& seq_within(seg.ack, tcb->snd.una-(1U<<31), tcb->snd.una-(1<<29))){
			print("stateless hog %I.%d->%I.%d f %ux %lux - %lux - %lux\n",
				source, seg.source, dest, seg.dest, seg.flags,
				tcb->snd.una-(1U<<31), seg.ack, tcb->snd.una-(1<<29));
...
tcpoutput(Conv *s)
{
	...
	Block *hbp, *bp;
	int sndcnt, n;
	u32int ssize, dsize, usable, sent;
	Fs *f;
	Tcppriv *tpriv;
	uchar version;
