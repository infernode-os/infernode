...
/*
 * DIVERGENCE FROM UPSTREAM: the embedded Lock is named.
 *
 * Upstream writes "Lock;" and passes the container to ilock(). Under
 * clang that declares nothing, so ialloc.bytes would land at offset 0
 * and every ilock(&ialloc) would write lock state straight over the
 * byte counter. See os/bcm2837/README.md.
 */
struct
{
	Lock	l;
