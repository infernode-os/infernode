/*
 * Imported from upstream Inferno os/port/chan.c.
 *
 * DIVERGENCE FROM UPSTREAM: Chan, Cname and Mhead embed named Ref/Lock
 * members rather than anonymous ones, so x->ref becomes x->r.ref and
 * incref(x) becomes incref(&x->r). Plan 9's "[index] value" designated
 * initialisers are also written the ISO way, "[index] = value".
