/*
 * Imported from upstream Inferno os/port/dev.c.
 *
 * DIVERGENCE FROM UPSTREAM: the local Dir-plus-slop struct in
 * devdirread names its embedded Dir. See os/bcm2837/README.md.
 */
...
devdirread(Chan *c, char *d, long n, Dirtab *tab, int ntab, Devgen *gen)
{
	long m, dsz;
	/*
	 * A Dir followed by room for the variable-length strings
	 * convD2M will pack after it. The Dir must stay at offset 0 --
	 * naming the member does not move it, so &dir.d is the same
	 * address the anonymous form produced.
	 */
	struct{
		Dir	d;
		char	slop[100];
