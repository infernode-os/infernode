...
struct Draw
{
	 ...
	 * Named, not anonymous.
	 *
	 * This tree de-anonymises Plan 9 C by hand rather than compiling
	 * with -fms-extensions: that flag only downgrades the
	 * container-passing to a warning WITHOUT performing the
	 * conversion, which is correct only when the member sits at
	 * offset 0. Tested at offset 16 it wrote to offset 0, clobbering
	 * an unrelated field and leaving the lock untouched -- silent
	 * corruption on every acquisition.
	 */
	QLock	l;
	int		clientid;
	int		nclient;
...
drawread(Chan *c, void *a, long n, vlong off)
{
	...
	case Qrefresh:
		...
		for(;;){
			...
			qunlock(&sdraw.l);
			if(waserror()){
				qlock(&sdraw.l);	/* restore lock for waserror() above */
				nexterror();
			}
 ...
 * source and mask, and which the platform defines (os/fb/screen.c,
