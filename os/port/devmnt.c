/*
 * IMPORTED from upstream Inferno (inferno-os/inferno-os), os/port/devmnt.c.
 ...
 *
 * #M -- the mount driver: the 9P CLIENT.
 *
 * Everything this kernel reaches through a mount goes through here.
 * Without it sys->mount() cannot work at all, which in an Inferno
 * kernel is close to saying that nothing composed of separate parts
 * can work: a service written as a program is reachable only if the
 * kernel can mount it.
 *
 * It arrived late in this port for the same reason #| did -- nothing
 * had needed it yet -- and it announced itself the same way, as
 * "panic: devno M 0x4d" the first time a Dis program tried to publish
 * a file interface. That is the twin recorded under "Smaller things
 * still open" in os/bcm2837/README.md, and the trigger was the USB
 * Ethernet driver: the driver-placement decision puts class drivers
 * outside the kernel, and a driver outside the kernel is reached by
 * mounting it.
 *
 * Dialect: one anonymous "Lock;" in Mntalloc became "Lock l;", and 38
 * container-passing call sites -- lock(c) for lock(&c->l), incref(m->c)
 * for incref(&m->c->r) and so on -- were made explicit, which is the
 * convention the rest of os/port already follows here. See the
 * decision recorded in os/bcm2837/README.md for why this is done by
 * hand rather than with -fplan9-extensions.
 */
#include	"u.h"
#include	"../port/lib.h"
#include	"mem.h"
