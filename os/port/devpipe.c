 ...
 * #| -- the pipe device.
 *
 * IMPORTED from emu/port/devpipe.c, which is the same device over the
 * same Queue machinery a native kernel already has. Only the include
 * preamble differs: emu's sources start at dat.h and pull in their own
 * lib9.h, while a native kernel leads with u.h and takes lib.h and
 * error.h from os/port.
 *
 * Without it sysfile.c's syspipe() indexes devtab through
 * devno('|', 0), and a devno miss with user==0 PANICS -- so a Limbo
 * program calling sys->pipe() took the kernel down. Found by typing a
 * backquote substitution at the shell:
 *
 *     ; y=`{echo one two three}
 *     panic: devno | 0x7c
 *
 * It is also what pipelines need, so the shell is only half a shell
 * without it.
 */
#include	"u.h"
#include	"../port/lib.h"
...
/*
 * As with devprog: emu's Dev has one init hook where a native kernel's
 * has reset, init and shutdown, so the positional initializer needs
 * reshaping or every entry after the mismatch is a function pointer of
 * the wrong type.
 */
Dev pipedevtab = {
	'|',
