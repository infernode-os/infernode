/*
 * #t -- serial ports as files: /dev/eiaN, eiaNctl, eiaNstatus.
 *
 * Inferno's os/port/devuart.c (in this tree's history until e3914b1c7,
 * identical upstream, MIT), reinstated for the Bluetooth work: the
 * radio on a Pi 3 hangs off the PL011, and a program that speaks HCI
 * to it needs the UART as a file. The interface is man/3/eia, which
 * the hosted emulator's deveia serves over host ttys, so Limbo written
 * against /dev/eia0 here runs unchanged there.
 *
 * The hardware is behind PhysUart (uart.h): one per kind of UART, in
 * the board's physuart[] table, in the order that numbers them.
 *
 * Divergences from upstream, each for a reason:
 *
 ...
 *   What this file adds to the console is INPUT -- the receive
 *   interrupt feeding kbdq, which replaces a 10ms polling kproc whose
 *   FIFO-sized losses AGENTS.md had to warn about.
 * - The console Uart keeps whatever putc its PhysUart's pnp() set, so
 *   the board decides how to echo; kbdcr2nl is the default.
 * - uartmouse/uartsetmouseputc are gone: no serial mice here.
 * - A ctl verb the table does not know is an error (Ebadarg), where
 *   upstream ignored it. A typo in a baud command must not read as a
 *   UART that took the setting and did not honour it.
 * - A byte that arrives with the staging ring full is counted in berr
 *   rather than dropped without trace; status reports it.
 */
#include	"u.h"
#include	"../port/lib.h"
#include	"mem.h"
...
uartreset(void)
{
	...
	for(i = 0; i < uartnuart; i++){
		...
		if(p->console || p->special){
			if(uartenable(p) != nil){
				/*
				 * The console's received bytes go straight to
				 * the keyboard queue at interrupt time, through
				 * the putc the board chose (echo is its business);
				 * kbdcr2nl if it chose nothing. kbdq itself stays
				 * devcons's: it allocated it, and the debug keys,
				 * the raw mode and consread all hold it.
				 */
				if(p->console && p->putc == nil)
					p->putc = kbdcr2nl;
				p->opens++;
			}
		}
