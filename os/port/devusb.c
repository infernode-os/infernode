/*
 * #u -- the USB device framework.
 *
 * IMPORTED VERBATIM from Plan 9 4th edition, sys/src/9/pc/devusb.c, via the
...
hciprobe(int cardno, int ctlrno)
{
	...
	hp->ctlrno = ctlrno;
	hp->tbdf = BUSUNKNOWN;	/* not a PCI device */
...
epctl(Ep *ep, Chan *c, void *a, long n)
{
	...
	case CMnewdev:
		...
		/*
		 * Chan.aux is void*; kstrdup takes char**. Plan 9's
		 * compiler accepts the mismatch and clang does not -- and
		 * this build escalates it to an error deliberately, since
		 * it is the same class of silent type confusion that the
		 * anonymous-member handling exists to catch.
		 */
