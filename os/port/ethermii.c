/*
 * MII: finding a PHY on a MAC's management bus, negotiating a link
 * with it and reading back what was agreed.
 *
 * 9front's sys/src/9/port/ethermii.c (MIT: Copyright © 2021 Plan 9
 * Foundation, Copyright © 9front authors). Changed only where Plan 9
 * C is not C: the Mii's anonymous QLock is named, and a parameter has
 * a name. Its one user is ../bcm2711/ethergenet.c, so the boards that
 * have no such MAC leave it out (PORTSKIP in the harness).
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
/* hook for devmii */
static void dummy(Mii *m){ USED(m); }
