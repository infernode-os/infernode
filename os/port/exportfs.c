/*
 * exportfs -- serve a namespace over 9P.
 *
 * IMPORTED from emu/port/exportfs.c, with the same include change every
 * other import here needed: emu's sources start at dat.h and pull in
 * their own lib9.h, a native kernel leads with u.h and takes lib.h and
 * error.h from os/port.
 *
 * This is the other half of what a window system needs, and it pairs
 * with devsrv. #s lets a Limbo program serve names in ITS OWN
 * namespace; exportfs lets it hand a whole namespace to another
 * process down a pipe. wmsrv gives each client a private namespace
 * that way, which is why a client sees only its own window and cannot
 * reach anyone else's -- the isolation is the namespace, not a check.
 *
 * "exportfs not imported" was where wm/wm stopped once file2chan
 * worked.
 */
#include	"u.h"
#include	"../port/lib.h"
...
/*
 * Qid translation, carried over from emu/port/dat.h.
 *
 * An exported namespace is a UNION of things from different devices,
 * and two of them can hand out the same qid path -- they are only
 * unique per device. A client on the far end of the pipe sees one
 * namespace and would take two identical paths to be the same file.
 * So each (type, dev, path) is mapped to a fresh path that is unique
 * across the export, and the table below is that mapping.
 *
 * Defined here rather than in os/port/portdat.h because exportfs is
 * the only thing in this kernel that has any use for them, and a
 * structure in a shared header invites a second, subtly different
 * user.
 */
struct Uqid
{
...
/*
 * The Qid translation itself, folded in from emu/port/uqid.c.
 *
 * It is a separate file there because several devfs-* servers use it.
 * Here exportfs is the only user, and folding it in keeps Uqid and
 * Uqidtab private to the file that has a use for them rather than
 * putting two more structures into a shared header.
 *
 * It sits ABOVE the rest of exportfs, not at the end, so that every
 * definition precedes its use -- appending it would have left
 * uqidalloc and mkuqid implicitly declared, which is how mkuqid ended
 * up returning int and assigning to a Qid.
 */
/*
...
exportproc(void *a)
{
	...
	for(;;){
		...
		for(n=0;; n++){	/* we don't use smalloc, to avoid memset */
			...
			/*
			 * osenter/osleave bracket a call that blocks in
			 * the HOST, so emu's scheduler can run something
			 * else meanwhile. There is no host here and no
			 * scheduler to tell, so the bracket goes and the
			 * sleep becomes the kernel's own.
			 *
			 * return0 is the portable "no condition, just
			 * wait out the interval" predicate that tsleep
			 * takes.
			 */
		...
		/*
		 * Wakeup and Sleep are emu's names for these. It spells
		 * them with capitals because the host's libc already has
		 * a sleep(); a kernel has no libc to collide with, so
		 * they are simply wakeup() and sleep() here.
		 */
		wakeup(&exq.rwait);
	}
