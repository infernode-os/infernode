/*
 * PCI: finding what is on the bus, sizing and placing its windows,
 * capabilities, MSI, power states.
 *
 * 9front's sys/src/9/port/pci.c (MIT: Copyright © 2021 Plan 9
 * Foundation, Copyright © 9front authors). Changed only where Plan 9
 * C is not C (parameters have names), and for a delay() this kernel
 * does not have. The boards with no PCI leave it out (PORTSKIP in the
 * harness); the host bridges are ../virt/pciecam.c, which QEMU can
 * exercise, and ../bcm2711/pcibcm.c, which nothing can yet.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
	...
	for(dno = 0; dno <= pcimaxdno; dno++){
		...
		for(fno = 0; fno <= maxfno; fno++){
			...
			case 0x11:		/* signal processing controllers */
				...
				rno = PciBAR0;
				for(i = 0; i < nelem(p->mem); i++) {
					p->mem[i].bar = (u32int)pcicfgr32(p, rno);
				...
				p->rom.bar = (u32int)pcicfgr32(p, PciEBAR0);
			...
			case 0x06:		/* bridge device */
				/* cardbus bridge? */
				if(p->ccru == 0x07){
					p->mem[0].bar = (u32int)pcicfgr32(p, PciBAR0);
				...
				p->rom.bar = (u32int)pcicfgr32(p, PciEBAR1);
