/*
 * PCI: configuration space, as any host bridge's driver and any PCI
 * device's driver see it.
 *
 * 9front's sys/src/9/port/pci.h (MIT: Copyright © 2021 Plan 9
 * Foundation, Copyright © 9front authors), unchanged; what this tree
 * adds is below the line at the end.
 */
/*
 * PCI support code.
 */
 ...
 * ---- this tree's ----
 *
 * What a host bridge's driver supplies. 9front declares the first
 * three in each architecture's fns.h and reaches the fourth through
 * intrenable's tbdf argument; here a machine with no PCI compiles none
 * of this, so they live with the rest.
 *
 *	pcicfgrw*	one configuration-space access; read says which
 *	pciintrenable	deliver this device's interrupt to f(ureg, a). How --
 *			a wire to the interrupt controller (../virt/pciecam.c)
 *			or a message the bridge turns into one
 *			(../bcm2711/pcibcm.c) -- is the bridge's business.
