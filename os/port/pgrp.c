/*
 * Imported from upstream Inferno os/port/pgrp.c.
 *
 * DIVERGENCE FROM UPSTREAM: Pgrp, Fgrp, Egrp and Mhead each embed a
 * named "Ref r" rather than an anonymous Ref, so x->ref becomes
 * x->r.ref and incref(x) becomes incref(&x->r). See
