/*
 * Imported from upstream Inferno os/port/qio.c.
 *
 * DIVERGENCE FROM UPSTREAM: Queue's embedded Lock is named, so the
 * container-passes ilock(q) become ilock(&q->l). Queue embeds only a
 * Lock, so every site here is unambiguous -- unlike Chan and Fgrp,
 * which embed both a Lock and a Ref. See os/bcm2837/README.md.
 */
