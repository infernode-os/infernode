/*
 * Imported from upstream Inferno os/port/sysfile.c.
 *
 * DIVERGENCE FROM UPSTREAM: Chan and Fgrp embed named Lock and Ref
 * members rather than anonymous ones, so lock(c) becomes lock(&c->l)
 * and incref(c) becomes incref(&c->r).
 *
 * Both structs embed BOTH a Lock and a Ref, so which member a given
 * call site wants was chosen by the TARGET parameter type reported by
 * the compiler, not by the source type -- 21 sites wanted the Lock and
 * one wanted the Ref. See os/bcm2837/README.md.
 */
