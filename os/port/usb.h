 ...
 *
 * IMPORTED VERBATIM from Plan 9 4th edition, sys/src/9/port/usb.h, via the
...
/*
 * Supplied here rather than imported: HOWMANY and BUSUNKNOWN come from
 * Plan 9 headers this kernel does not have (BUSUNKNOWN is a PCI bus
 * address, and there is no PCI on a BCM2837 -- the value only ever
 * means "not a PCI device").
 */
#ifndef HOWMANY
...
{
	/*
	 * De-anonymized. Upstream writes "ISAConf;" and "Hciimpl;" --
	 * Plan 9 C anonymous struct members, which clang does not accept
	 * (no -fplan9-extensions in any clang on this host, and
	 * -fms-extensions only gets part way). os/port is de-anonymized by
	 * hand throughout and these follow it.
	 *
	 * ISAConf is the PC ISA bus configuration struct and does not exist
	 * in this kernel -- there is no ISA bus on a BCM2837. Only three of
	 * its fields are ever touched, so they are spelled out rather than
	 * dragging in a struct that describes hardware this machine does
	 * not have.
	 *
	 * Hciimpl's members are FLATTENED rather than named, so that every
	 * "hp->init", "hp->aux" and so on in devusb.c and usbdwc.c keeps
	 * working untouched. Naming it would have meant editing thirty-odd
	 * call sites across two files we want to stay diffable against
	 * Plan 9.
	 */
	char*	type;				/* controller name, e.g. "dwcotg" */
	uintptr	port;				/* register base */
	int	irq;
