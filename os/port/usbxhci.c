/*
 * xHCI: the USB host controller interface of everything since USB 3.
 * On a Raspberry Pi 4 it is the VL805 behind the PCIe bridge, and the
 * four USB-A sockets; under QEMU it is -device qemu-xhci on the `virt`
 * machine, WHICH IS WHERE THIS DRIVER HAS RUN. It has never met a
 * VL805.
 *
 * Derived from 9front's sys/src/9/port/usbxhci.c (MIT: Copyright © 2021
 * Plan 9 Foundation, Copyright © 9front authors). The rings, the
 * command and event machinery, slots and endpoint contexts, recovery
 * and the isochronous scheduling are that driver's. What differs, and
 * why:
 *
 *   Plan 9 C. The Lock in a Ring, the QLock in an Epio and the Xhci in
 *   a Ctlr have names; parameters have names; register pointers are
 *   volatile, which Plan 9's compilers do not need to be told.
 *
 *   Memory the controller touches comes from pcidmaalloc()
 *   (../port/pci.h), because on a BCM2711 a PCI device reaches only
 *   the first gigabyte and the kernel's own allocator deliberately
 *   hands out memory above it first (../bcm/dmamem.c). 9front's
 *   mallocalign is not enough here.
 *
 *   For the same reason a caller's buffer may be unreachable. 9front
 *   bounces a user-space address; this kernel has no user space and
 *   bounces whatever pcidmaok() refuses, through a buffer kept with the
 *   endpoint. Control transfers' data stages and isochronous rings use
 *   the same buffer where 9front has a Block. `virt` can be told to
 *   refuse everything (pcibounce in the kernel's arguments), so that
 *   the path the Pi 4 depends on is run by the harness and not first
 *   on a board.
 *
 ...
 *   ports are powered at init because nothing will ask; and hubupdate()
 *   tells the controller a device is a hub when devusb's "hub N" says
 *   so, where 9front has usbd reopen ep0. The topology an xHC wants
 *   in a slot context -- route string, root port, TT hub -- is computed
 *   in devusb.c by 9front's arithmetic.
 *
 *   The interrupt is enabled by whoever found the controller
 *   (usbxhcipci.c, through the PCI bridge), not here.
 *
 * NOT DONE: SuperSpeed hubs (the walker does not speak their port
 * status or set their depth), streams, and MSI-X.
 */
...
#define dmaclean(p, n)	cachedwbse((p), (n))		/* 9front dmaclean(...): the device is about to READ this */
#define dmainval(p, n)	cachedwbinvse((p), (n))	/* 9front dmainval(...): the device has WRITTEN this, or will */
...
struct Ctlr
{
	Xhci	x;		/* MUST be first: usbxhcipci.c holds this as an Xhci* */

	volatile u32int	*opr;	/* operational registers */
	volatile u32int	*rts;	/* runtime registers */
	volatile u32int	*dba;	/* doorbell array */
	...
	int	nintrs;
	int	nslots;
	uint	superspeed;	/* bitmap of the root ports that are USB 3 */
...
struct Epio
{
	...
	/*
	 * A buffer the controller can reach (pcidmaalloc), where 9front has a
	 * Block: a control transfer's data stage, an iso stream's ring of
	 * frames, or the bounce for a bulk or interrupt transfer whose
	 * caller's buffer the controller cannot reach.
	 */
	uchar	*buf;
	ulong	nbuf;		/* allocated */
	uchar	*rp;		/* control: what epread has not yet taken */
	uchar	*wp;
...
uframe(Ctlr *ctlr)
{
	u32int uf;
	do {
		uf = (ctlr->rts[MFINDEX] & (1<<14)-1) |
			(ctlr->uframe & ~((1<<14)-1));
	} while((int)(uf - ctlr->uframe) < 0);
	return uf;
}

/*
 * Memory the controller reads and writes: from wherever this machine's
 * PCI devices can reach (pcidmaalloc, ../port/pci.h), zeroed. 9front
 * uses mallocalign with a span so that nothing crosses a page (or, for
 * a ring, 64K); here anything over a cache line is page-aligned, and
 * nothing that must not cross is bigger than a page. The size comes
 * back at free time because the Pi's arena wants it.
 */
static void*
xmemalloc(ulong size)
{
	return pcidmaalloc(size, size <= 64 ? 64 : BY2PG);
}

static void
xmemfree(void *p, ulong size)
{
	if(p != nil)
		pcidmafree(p, size);
}
...
freering(Ring *r)
{
	if(r == nil)
		return;
	if(r->base != nil){
		dmainval(r->base, 4*4<<r->shift);
		xmemfree(r->base, 4*4<<r->shift);
	}
	if(r->residue != nil)
		free(r->residue);
	memset(r, 0, sizeof(*r));
...
initring(Ctlr *ctlr, Ring *r, int shift)
{
	...
	r->mask = (1<<shift)-1;
	r->rp = r->wp = 0;
	r->base = xmemalloc(4*4<<shift);
	if(r->base == nil){
		freering(r);
		error(Enomem);
	}
	dmaclean(r->base, 4*4<<shift);
	return r;
}
...
resetring(Ring *r)
{
	...
	flushring(r);
	r->rp = r->wp;
	pa = (*r->ctlr->x.dmaaddr)(&r->base[4*(r->wp & r->mask)]) | ((~r->wp>>r->shift) & 1);
	iunlock(&r->l);
...
xecp(Xhci *xc, uchar id, volatile u32int *p)
{
	u32int x;
	volatile u32int *e;
...
xhcishutdown(Hci *hp)
{
	...
	for(i=100; (ctlr->opr[USBSTS] & HCH) == 0 && i >= 0; i--)
		delay(1);
	/* the interrupt stays enabled: this kernel has no intrdisable, and interrupt() ignores a controller with no event ring */
}
...
release(Ctlr *ctlr)
{
	...
	for(i=0; i<nelem(ctlr->er); i++){
		freering(&ctlr->er[i]);
		xmemfree(ctlr->erst[i], 4*4);
		ctlr->erst[i] = nil;
	}
	free(ctlr->port), ctlr->port = nil;
	free(ctlr->slot), ctlr->slot = nil;
	xmemfree(ctlr->dcba, (1+ctlr->nslots)*sizeof(ctlr->dcba[0])), ctlr->dcba = nil;
	xmemfree(ctlr->sba, ctlr->nscratch*8), ctlr->sba = nil;
	if(ctlr->sbp != nil){
		dmainval(ctlr->sbp, ctlr->nscratch*ctlr->pagesize);
		xmemfree(ctlr->sbp, ctlr->nscratch*ctlr->pagesize);
		ctlr->sbp = nil;
	}
}
...
xhciinit(Hci *hp)
{
	...
	ctlr = hp->aux;
	ctlr->opr = &ctlr->x.mmio[(ctlr->x.mmio[CAPLENGTH]&0xFF)/4];
	ctlr->dba = &ctlr->x.mmio[ctlr->x.mmio[DBOFF]/4];
	ctlr->rts = &ctlr->x.mmio[ctlr->x.mmio[RTSOFF]/4];
	...
	/* the interrupt was enabled by whoever found the controller (usbxhcipci.c): 9front does it here */
	...
	ctlr->csz = (ctlr->x.mmio[HCCPARAMS] & CSZ) != 0;
	ctlr->pagesize = (ctlr->opr[PAGESIZE] & 0xFFFF) << 12;

	ctlr->nscratch = (ctlr->x.mmio[HCSPARAMS2] >> 27) & 0x1F | ((ctlr->x.mmio[HCSPARAMS2] >> 21) & 0x1F)<<5;
	ctlr->nintrs = (ctlr->x.mmio[HCSPARAMS1] >> 8) & 0x7FF;
	ctlr->nslots = (ctlr->x.mmio[HCSPARAMS1] >> 0) & 0xFF;
	hp->nports = (ctlr->x.mmio[HCSPARAMS1] >> 24) & 0xFF;

	print("usbxhci %llux: csz=%d pagesize=%d nscratch=%d nintrs=%d nslots=%d nports=%d\n", ctlr->x.base,
	...
	while((x = xecp(&ctlr->x, 2, x)) != nil){
		...
		while(j--){
			...
			if(memcmp(pp->spec, "USB ", 4) == 0 && pp->proto >= 0x0300)
				ctlr->superspeed |= 1<<(i-1);
			i++;
		}
	}

	ctlr->slot = malloc((1+ctlr->nslots)*sizeof(ctlr->slot[0]));
	ctlr->dcba = xmemalloc((1+ctlr->nslots)*sizeof(ctlr->dcba[0]));
	if(ctlr->slot == nil || ctlr->dcba == nil)
		error(Enomem);
	if(ctlr->nscratch != 0){
		ctlr->sba = xmemalloc(ctlr->nscratch*8);
		ctlr->sbp = xmemalloc(ctlr->nscratch*ctlr->pagesize);
		if(ctlr->sba == nil || ctlr->sbp == nil)
			error(Enomem);
		for(i=0, p = ctlr->sbp; i<ctlr->nscratch; i++, p += ctlr->pagesize){
			memset(p, 0, ctlr->pagesize);
			ctlr->sba[i] = (*ctlr->x.dmaaddr)(p);
		}
		dmaclean(ctlr->sbp, ctlr->nscratch*ctlr->pagesize);
		dmaclean(ctlr->sba, ctlr->nscratch*8);
		ctlr->dcba[0] = (*ctlr->x.dmaaddr)(ctlr->sba);
	} else {
		ctlr->dcba[0] = 0;
	}
	...
	dmaclean(ctlr->dcba, (1+ctlr->nslots)*sizeof(ctlr->dcba[0]));
	setrptr(&ctlr->opr[DCBAAP], (*ctlr->x.dmaaddr)(ctlr->dcba));
	...
	for(i=0; i<ctlr->nintrs; i++){
		...
		ctlr->erst[i] = xmemalloc(4*4);
		if(ctlr->erst[i] == nil)
			error(Enomem);
		*((u64int*)ctlr->erst[i]) = (*ctlr->x.dmaaddr)(ctlr->er[i].base);
		ctlr->erst[i][2] = ctlr->er[i].mask+1;
		ctlr->erst[i][3] = 0;
		dmaclean(ctlr->erst[i], 4*4);

		/* just one segment */
		irs[ERSTSZ] = 1 | (irs[ERSTSZ] & ERSTSZ_PRES);
		setrptr(&irs[ERDP], (*ctlr->x.dmaaddr)(ctlr->er[i].base) | EHB);
		setrptr(&irs[ERSTBA], (*ctlr->x.dmaaddr)(ctlr->erst[i]) |
	...
	/*
	 * Power the root ports. 9front's usbd does it with a hub request
	 * (its devusb turns SetPortFeature(PORT_POWER) on a root hub into
	 * portpower()); this tree's root hub is Plan 9's toy and its walker
	 * never asks. Most controllers have no port power switching and
	 * come out of reset with PP already set.
	 */
	for(i=0; i<hp->nports; i++)
		if((ctlr->port[i].reg[PORTSC] & PP) == 0)
			ctlr->port[i].reg[PORTSC] = (ctlr->port[i].reg[PORTSC] & ~(RW1CS|RW1S)) | PP;
...
recover(void *arg)
{
	...
	print("usbxhci %llux: need recover: USBSTS=%ux, er stopped=%d\n",
		ctlr->x.base, ctlr->opr[USBSTS], ctlr->er->stopped);
...
queuetd(Ring *r, u32int c, u32int s, u64int p, Wait *w)
{
	...
	if((x & r->mask) == r->mask){
		...
		*(u64int*)td = (*r->ctlr->x.dmaaddr)(r->base);
		td[2] = 0;
...
interrupt(Ureg *ur, void *arg)
{
	...
	for(x = ring->rp;; x=++ring->rp){
		...
		case ER_TRANSFER:
			...
			break;
		case ER_MFINDEXWRAP:
			ctlr->uframe = (ctlr->rts[MFINDEX] & (1<<14)-1) | 
				(ctlr->uframe+(1<<14) & ~((1<<14)-1));
			break;
		case ER_HCE:
			iprint("usbxhci %llux: host controller error: %ux %ux %ux %ux\n",
				ctlr->x.base, td[0], td[1], td[2], td[3]);
			ctlr->er->stopped = 1;
			wakeup(&ctlr->recover);
			return;
		case ER_PORTSC:
		...
		default:
			iprint("usbxhci %llux: event %ud: %ux %ux %ux %ux\n",
				ctlr->x.base, x, td[0], td[1], td[2], td[3]);
		}
	}

	setrptr(&irs[ERDP], (*ctlr->x.dmaaddr)(td) | EHB);
}
...
freeslot(Slot *slot)
{
	...
	if(slot->id > 0){
		...
		&& ctlr->slot[slot->id] == slot){
			...
			dmainval(slot->obase, 32*32 << ctlr->csz);
			ctlr->dcba[slot->id] = 0;
			dmaclean(&ctlr->dcba[slot->id], sizeof(ctlr->dcba[0]));
	...
	freering(&slot->epr[0]);
	if(slot->ctlr != nil){
		xmemfree(slot->ibase, 32*33 << slot->ctlr->csz);
		xmemfree(slot->obase, 32*32 << slot->ctlr->csz);
	}
	free(slot);
}
...
allocslot(Ctlr *ctlr, Udev *dev)
{
	...
	if(ctlr->slot == nil)
		error(Exrecover);
	slot->ibase = xmemalloc(32*33 << ctlr->csz);
	slot->obase = xmemalloc(32*32 << ctlr->csz);
	...
	dmaclean(slot->obase, 32*32 << ctlr->csz);
	ctlr->dcba[slot->id] = (*ctlr->x.dmaaddr)(slot->obase);
	dmaclean(&ctlr->dcba[slot->id], sizeof(ctlr->dcba[0]));
...
epclose(Ep *ep)
{
	...
	if(slot != nil && ep->nb > 0 && (io[OREAD].ring != nil || io[OWRITE].ring != nil)){
		/* (input) control context */
		...
		dmaclean(slot->ibase, 32*33 << ctlr->csz);
		ctlrcmd(ctlr, CR_CONFIGEP | (slot->id<<24), 0,
			(*ctlr->x.dmaaddr)(slot->ibase), nil);
		dmainval(slot->obase, 32*32 << ctlr->csz);

		freering(io[OREAD].ring);
		freering(io[OWRITE].ring);
	}
	xmemfree(io[OREAD].buf, io[OREAD].nbuf);
	xmemfree(io[OWRITE].buf, io[OWRITE].nbuf);
	free(io);
}
...
initepctx(Ctlr *ctlr, u32int *w, Ring *r, Ep *ep)
{
	...
	if(ep->ttype != Tiso)
		w[1] |= 3<<1;
	*((u64int*)&w[2]) = (*ctlr->x.dmaaddr)(r->base) | 1;
	w[4] = 2*ep->maxpkt;
...
initisoio(Epio *io, Ep *ep)
{
	...
	io->nbuf = (io->ring->mask+1)*io->tdsz;
	io->buf = xmemalloc(io->nbuf);
	if(io->buf == nil){
		io->nbuf = 0;
		error(Enomem);
	}
...
initep(Ep *ep)
{
	...
	if(ep->nb == 0){
		 ...
		 * command.
		 *
		 * 9front leaves the ep0 flag set for that command too. The
		 * specification says Configure Endpoint takes A0 set and A1
		 * CLEAR (4.6.6), QEMU's controller enforces it with a TRB
		 * Error -- which is how this was found -- and Linux sends
		 * the slot flag alone. So does this.
		 */
		if(ep->dev->nports > 0){
			cmd = CR_CONFIGEP;
			ic[1] = 1;
...
/*
 * A device has just been declared a hub, or had its port count and TT
 * said (devusb's "hub N ttt mtt"). The controller must know before it
 * will address anything behind the hub, and those fields of a slot
 * context are taken only by Configure Endpoint -- which is what initep
 * issues for an ep0 whose device has ports. 9front reaches the same
 * code by having usbd close and reopen ep0 around the ctl write; this
 * tree's walker holds ep0 open throughout, so devusb calls here.
 */
static void
hubupdate(Ep *ep)
{
	...
	if(ep->dev->isroot || ep->nb != 0 || ep->aux == nil)
		return;
	slot = ep->dev->aux;
	if(slot == nil || slot->dev != ep->dev)
		return;
	initep(ep);
}
...
isoread(Ep *ep, uchar *p, long n)
{
	...
	for(i = io->frame0; (int)(io->ring->rp - io->rp0) > 0 && n > 0; i++) {
		...
		if(m > 0){
			d = io->buf + (i&io->ring->mask)*io->tdsz;
	...
	for(i = io->frame;; i++){
		...
		d = io->buf + (i&io->ring->mask)*io->tdsz;
		dmaclean(d, m);
		queuetd(io->ring, TR_ISOCH | (i*uf/8 & 0x7ff)<<20 | TR_IOC, m,
			(*ctlr->x.dmaaddr)(d), nil);
	}
	io->frame = i;
...
/*
 * A buffer of at least n bytes the controller can reach, kept with the
 * endpoint and grown as needed. Called with io locked.
 */
static uchar*
iobuf(Epio *io, ulong n)
{
	if(n == 0)
		n = 1;
	if(io->nbuf < n){
		xmemfree(io->buf, io->nbuf);
		io->nbuf = 0;
		io->buf = xmemalloc(n);
		if(io->buf == nil)
			error(Enomem);
...
epread(Ep *ep, void *va, long n)
{
	...
	qlock(&io->ql);
	if(waserror()){
		dmainval(io->ring->ctx, 8*4 << ctlr->csz);
		qunlock(&io->ql);
		nexterror();
	}
	...
	/*
	 * 9front bounces a user-space buffer; this kernel has no user
	 * space, and what it has instead is memory a PCI device may not
	 * be able to reach (pcidmaok, ../port/pci.h).
	 */
	dp = p;
	if(!pcidmaok(p, n)){
		dp = iobuf(io, n);
	...
	dmainval(dp, n);
	queuetd(io->ring, TR_NORMAL | TR_IOC, n, (*ctlr->x.dmaaddr)(dp), w);
...
epwrite(Ep *ep, void *va, long n)
{
	...
	if(ep->ttype == Tctl){
		...
		/* SET_ADDRESS: the controller did it, at epopen (Address Device) */
		...
		if(waserror()){
			...
			ring->pending = nil;
			iunlock(&ring->l);
			dmainval(ring->ctx, 8*4 << ctlr->csz);
			qunlock(&io->ql);
			nexterror();
		}
		io->rp = io->wp = nil;
...
/*
 * The root hub. 9front presents two -- a USB 2 one and a USB 3 one,
 * the second answering in a USB 3 hub's different status layout -- and
 * its usbd knows both. This tree's devusb has Plan 9's "toy hub" and a
 * walker in Limbo that knows one layout, so every root port answers in
 * that one, with a bit of this tree's own (HPsuper) for a port that
 * came up at SuperSpeed. The change bits do not fit the toy hub's 16-bit
 * reply and the walker polls HPpresent instead.
 */
static int
portstatus(Hci *hp, int port)
{
	...
	if((ps & HPreset) == 0){
		...
		case 3:
			...
			break;
		default:
			if((ps & HPpresent) != 0 && (ctlr->superspeed & (1<<(port-1))) != 0)
				ps |= HPsuper;
			break;
		}
	}
...
/*
 * Plan 9's portreset is synchronous -- usbdwc.c's holds the reset and
 * returns when the port is enabled -- and the walker reads the status
 * straight after. An xHC runs the reset itself and says when (PRC), so
 * wait for that, clear it, and give the device its recovery time.
 */
static int
portreset(Hci *hp, int port, int on)
{
	...
	*portsc = (*portsc & ~(RW1CS|RW1S)) | PRC;
	tsleep(&up->sleep, return0, nil, 20);
	return 0;
}

/*
 * "dump" written to #u/usb/ctl: what the controller and its ports say,
 * and how much went through a bounce buffer -- which on a Raspberry Pi 4
 * should be most bulk transfers, on `virt` none unless pcibounce, and
 * is how the harness knows that path ran.
 */
static void
dump(Hci *hp)
{
	...
	if(ctlr->port == nil){
		print("usbxhci %llux: not initialised\n", ctlr->x.base);
		return;
	}
	n = 0;
	qlock(&ctlr->slotlock);
	if(ctlr->slot != nil)
		for(i = 1; i <= ctlr->nslots; i++)
			if(ctlr->slot[i] != nil)
				n++;
	qunlock(&ctlr->slotlock);
	print("usbxhci %llux: usbsts %#ux, %d of %d slots in use, event ring %s\n",
		ctlr->x.base, ctlr->opr[USBSTS], n, ctlr->nslots, ctlr->er->stopped ? "STOPPED" : "running");
	for(i = 0; i < hp->nports; i++)
...
physaddr(void *va)
{
	return pcibusaddr(va);
}

Xhci*
xhcialloc(volatile u32int *mmio, u64int base, u64int size)
{
	Ctlr *ctlr;
