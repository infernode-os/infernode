/*
 * Imported from upstream Inferno os/port/xalloc.c.
 *
 * DIVERGENCE FROM UPSTREAM: the embedded Lock is named (Lock l) and its
 * call sites are explicit (&xlists.l). Upstream uses a Plan 9 anonymous
 * struct member and passes the container.
 *
 * This is not stylistic. Clang has no working equivalent:
 * -fplan9-extensions is a GCC flag it does not implement, and
 * -fms-extensions only downgrades the container-pass to a warning
 * without performing the conversion. Left as upstream writes it, the
 * anonymous "Lock;" declares nothing at all, so Hole hole[Nhole] lands
 * at offset 0 and ilock(&xlists) writes lock state directly into
 * hole[0] -- silently destroying the free list. That is precisely how
 * this was found: xalloc returned nil for every request.
 */
...
struct Xalloc
{
	Lock	l;		/* was an anonymous Plan 9 embed; see below */
	Hole	hole[Nhole];
	Hole*	flist;
	Hole*	table;
};
