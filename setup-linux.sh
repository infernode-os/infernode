...
setup_anthropic() {
    ...
    # the right backend. Model is left blank - user picks one in Settings.
    mkdir -p "$ROOT/lib/ndb"
    cat > "$ROOT/lib/ndb/llm" <<EOF
mode=local
backend=api
...
dial=
EOF
    ok "Config saved to lib/ndb/llm"

    printf "\n"
    ok "Anthropic backend configured."
    printf "  Pick a model in Settings (Lucifer).\n"
    printf "${DIM}  Common choices: claude-sonnet-4-5-20250929, claude-haiku-4-5-20251001${RESET}\n"
    return 0
}
...
setup_ollama() {
    ...
    # Write the runtime LLM config. The runtime reads /lib/ndb/llm (see
    # lib/sh/profile and lib/lucifer/boot.sh); the old lib/veltro/llm.cfg
    # path was a dead file - nothing read it.
    mkdir -p "$ROOT/lib/ndb"
    cat > "$ROOT/lib/ndb/llm" <<EOF
mode=local
backend=openai
