...
| `card.py` | none; a file-integrity check: every file under the card's `dis`, `lib`, `fonts` and `icons` by name and md5 against the build (`--root`). Finds the mis-named (a lost long-name entry leaves `mail~155.dis`), the missing and the stale. Run after writing a card and before a soak; `--fix` prints the repair | network console only | `md5sum`, `du` on `/n/dos` |
