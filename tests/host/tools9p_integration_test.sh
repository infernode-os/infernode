...
if emu_c "startup_privileged_path_rejected" 12 \
    "mkdir -p /mnt/factotum /mnt/ui/activity/0/presentation /mnt/msg /tmp/veltro/ftree /tmp/veltro/.ns /tmp/veltro/cow /tmp/veltro/tasks /tmp/veltro/browser /tmp/veltro/editor /tmp/veltro/shell /tmp/veltro/fractal /tmp/veltro/man; touch /mnt/factotum/ctl; touch /mnt/msg/ctl; touch /tmp/veltro/ftree/ctl; tools9p -p /mnt/factotum:rw -p /mnt/factotum/ctl:rw -p /mnt/ui:rw -p /mnt/ui/activity/0/presentation:rw -p /mnt/msg/ctl:rw -p /tmp/veltro/ftree:rw -p /tmp/veltro/.ns:rw -p /tmp/veltro/cow:rw -p /tmp/veltro/tasks:rw -p /tmp/veltro/browser:rw -p /tmp/veltro/editor:rw -p /tmp/veltro/shell:rw -p /tmp/veltro/fractal:rw -p /tmp/veltro/man:rw read; cat /tool/paths >[2] /dev/null"; then
    if echo "$OUTPUT" | grep -q "privileged -p path not grantable" && ! echo "$OUTPUT" | grep -q "^/mnt/factotum" && ! echo "$OUTPUT" | grep -q "^/mnt/ui" && ! echo "$OUTPUT" | grep -q "^/mnt/msg/ctl" && ! echo "$OUTPUT" | grep -q "^/tmp/veltro/ftree" && ! echo "$OUTPUT" | grep -q "^/tmp/veltro/.ns" && ! echo "$OUTPUT" | grep -q "^/tmp/veltro/cow" && ! echo "$OUTPUT" | grep -q "^/tmp/veltro/tasks" && ! echo "$OUTPUT" | grep -q "^/tmp/veltro/browser" && ! echo "$OUTPUT" | grep -q "^/tmp/veltro/editor" && ! echo "$OUTPUT" | grep -q "^/tmp/veltro/shell" && ! echo "$OUTPUT" | grep -q "^/tmp/veltro/fractal" && ! echo "$OUTPUT" | grep -q "^/tmp/veltro/man"; then
